...
#[tauri::command(async)]
pub fn emulator_upload_path_by_id(path: String, id: String) -> Result<String, Error> {
    // Open session connect
    let conn = get_session()?;
    // Get proxy with timeout
    let proxy = get_proxy_bot(&conn, TIMEOUT_SHORT);
    // Request
    let method = "EmulatorUploadPathById";
...
#[tauri::command(async)]
pub fn emulator_upload_url(url: String) -> Result<String, Error> {
    // Open session connect
    let conn = get_session()?;
    // Get proxy with timeout
    let proxy = get_proxy_bot(&conn, TIMEOUT_SHORT);
    // Request
    let method = "EmulatorUploadUrl";
...
#[tauri::command(async)]
pub fn emulator_upload_url_by_id(url: String, id: String) -> Result<String, Error> {
    // Open session connect
    let conn = get_session()?;
    // Get proxy with timeout
    let proxy = get_proxy_bot(&conn, TIMEOUT_SHORT);
    // Request
    let method = "EmulatorUploadUrlById";
