...
#[tauri::command(async)]
pub fn emulator_package_uninstall() -> Result<String, Error> {
...
#[tauri::command(async)]
pub fn emulator_package_uninstall_by_id(id: String) -> Result<String, Error> {
    // Open session connect
    let conn = get_session()?;
    // Get proxy with timeout
    let proxy = get_proxy_bot(&conn, TIMEOUT_SHORT);
...
#[tauri::command(async)]
pub fn emulator_package_uninstall_package(package: String) -> Result<String, Error> {
    // Open session connect
    let conn = get_session()?;
    // Get proxy with timeout
    let proxy = get_proxy_bot(&conn, TIMEOUT_SHORT);
...
#[tauri::command(async)]
pub fn emulator_package_uninstall_package_by_id(package: String, id: String) -> Result<String, Error> {
    // Open session connect
    let conn = get_session()?;
    // Get proxy with timeout
    let proxy = get_proxy_bot(&conn, TIMEOUT_SHORT);
