...
#[tauri::command(async)]
pub fn flutter_download_by_id(id: String) -> Result<String, Error> {
    // Open session connect
    let conn = get_session()?;
    // Get proxy with timeout
    let proxy = get_proxy_bot(&conn, TIMEOUT_LONG);
    // Request
    let method = "FlutterDownloadById";
