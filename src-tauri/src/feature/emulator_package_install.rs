...
#[tauri::command(async)]
pub fn emulator_package_install_path_by_id(path: String, id: String) -> Result<String, Error> {
    // Open session connect
    let conn = get_session()?;
    // Get proxy with timeout
    let proxy = get_proxy_bot(&conn, constants::TIMEOUT_SHORT);
    // Request
    let method = "EmulatorPackageInstallPathById";
...
#[tauri::command(async)]
pub fn emulator_package_install_url(url: String) -> Result<String, Error> {
    // Open session connect
    let conn = get_session()?;
    // Get proxy with timeout
    let proxy = get_proxy_bot(&conn, constants::TIMEOUT_SHORT);
    // Request
    let method = "EmulatorPackageInstallUrl";
...
#[tauri::command(async)]
pub fn emulator_package_install_url_by_id(url: String, id: String) -> Result<String, Error> {
    // Open session connect
    let conn = get_session()?;
    // Get proxy with timeout
    let proxy = get_proxy_bot(&conn, constants::TIMEOUT_SHORT);
    // Request
    let method = "EmulatorPackageInstallUrlById";
