...
pub fn emulator_terminal(is_root: bool) -> Result<String, Error> {
    ...
    let (result,): (String,) = match proxy.method_call(constants::DBUS_BOT_INTERFACE, method, (is_root,)) {
        Ok(value) => value,
...
pub fn emulator_terminal_by_id(is_root: bool, id: String) -> Result<String, Error> {
    ...
    let (result,): (String,) = match proxy.method_call(constants::DBUS_BOT_INTERFACE, method, (is_root, id)) {
        Ok(value) => value,
        Err(e) => Err(Error::Anyhow(e.into()))?,
    };
    Ok(result)
