    ...
    React.useEffect(() => {
        if (dialogSelectItemRun) {
            (async function () {
                ...
                try {
                    await Methods.emulator_package_run_package_by_id(false, dialogSelectItemRun.value, model.id);
    ...
    React.useEffect(() => {
        if (dialogSelectItemUninstall) {
            (async function () {
                ...
                setDialogBody(t('common.t_dialog_body_connection'));
                try {
                    await Methods.emulator_package_uninstall_package_by_id(dialogSelectItemUninstall.value, model.id);
                    setDialogState('success');
                    ...
                    <AvatarButton
                        ...
                        onClick={async () => {
                            ...
                            if (path) {
                                ...
                                if (unlisten) {
                                    try {
                                        await Methods.emulator_package_install_path_by_id(path, model.id);
                                        await unlisten();
                                        await new Promise(r => setTimeout(r, 500)); // animation delay
                                        setDialogState('success');
                                        setDialogBody(t('emulator.t_dialog_success_install'));
                                    } catch (e) {
                                        await unlisten();
                                        setDialogState('error');
                                        setDialogBody(t('common.t_dialog_body_error'));
                            ...
                            if (path) {
                                ...
                                if (unlisten) {
                                    try {
                                        ...
                                        await new Promise(r => setTimeout(r, 500)); // animation delay
                                        setDialogState('success');
                                        setDialogBody(t('emulator.t_dialog_success_upload'));
                                    } catch (e) {
                                        await unlisten();
                                        setDialogState('error');
                                        setDialogBody(t('common.t_dialog_body_error'));
