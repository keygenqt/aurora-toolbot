...
#[tauri::command(async)]
pub fn psdk_target_package_install_by_id(path: String, id: String) -> Result<String, Error> {
    // Open session connect
    let conn = get_session()?;
    // Get proxy with timeout
    let proxy = get_proxy_bot(&conn, constants::TIMEOUT_SHORT);
    // Request
    let method = "PsdkTargetPackageInstallById";
