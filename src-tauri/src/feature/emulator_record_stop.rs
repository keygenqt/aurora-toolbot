...
pub fn emulator_record_stop(stop_type: String) -> Result<String, Error> {
    ...
    let (result,): (String,) = match proxy.method_call(constants::DBUS_BOT_INTERFACE, method, (stop_type,)) {
        Ok(value) => value,
...
pub fn emulator_record_stop_by_id(stop_type: String, id: String) -> Result<String, Error> {
    ...
    let (result,): (String,) = match proxy.method_call(constants::DBUS_BOT_INTERFACE, method, (stop_type, id)) {
        Ok(value) => value,
        Err(e) => Err(Error::Anyhow(e.into()))?,
    };
    Ok(result)
